...
func TestLoadTemplates(t *testing.T) {
	t.Run("valid directory with templates", func(t *testing.T) {
		...
		require.NoError(t, os.MkdirAll(template1Dir, 0o750))
		require.NoError(t, os.WriteFile(filepath.Join(template1Dir, "config.yaml"), []byte("name: template1\ncategory: test"), 0o600))
		require.NoError(t, os.WriteFile(filepath.Join(template1Dir, "template.md"), []byte("# Template 1"), 0o600))

		template2Dir := filepath.Join(tmpDir, "template2")
		require.NoError(t, os.MkdirAll(template2Dir, 0o750))
		require.NoError(t, os.WriteFile(filepath.Join(template2Dir, "config.yaml"), []byte("name: template2\ncategory: test"), 0o600))
		require.NoError(t, os.WriteFile(filepath.Join(template2Dir, "template.md"), []byte("# Template 2"), 0o600))
	...
	t.Run("directory with incomplete templates", func(t *testing.T) {
		...
		require.NoError(t, os.WriteFile(filepath.Join(validDir, "template.md"), []byte("# Valid"), 0o600))

		missingConfigDir := filepath.Join(tmpDir, "missing-config")
		require.NoError(t, os.MkdirAll(missingConfigDir, 0o750))
		require.NoError(t, os.WriteFile(filepath.Join(missingConfigDir, "template.md"), []byte("# Missing Config"), 0o600))

		missingTemplateDir := filepath.Join(tmpDir, "missing-template")
		require.NoError(t, os.MkdirAll(missingTemplateDir, 0o750))
		require.NoError(t, os.WriteFile(filepath.Join(missingTemplateDir, "config.yaml"), []byte("name: missing"), 0o600))
...
func TestLoadTemplatesFromMultipleSources(t *testing.T) {
	t.Run("built-in and custom templates", func(t *testing.T) {
		...
		require.NoError(t, os.MkdirAll(tpl1Dir, 0o750))
		require.NoError(t, os.WriteFile(filepath.Join(tpl1Dir, "config.yaml"), []byte("name: builtin-1\ncategory: test\ndescription: Built-in template 1"), 0o600))
		require.NoError(t, os.WriteFile(filepath.Join(tpl1Dir, "template.md"), []byte("# Built-in 1"), 0o600))

		tpl2Dir := filepath.Join(builtInDir, "builtin-2")
		require.NoError(t, os.MkdirAll(tpl2Dir, 0o750))
		require.NoError(t, os.WriteFile(filepath.Join(tpl2Dir, "config.yaml"), []byte("name: builtin-2\ncategory: test\ndescription: Built-in template 2"), 0o600))
		require.NoError(t, os.WriteFile(filepath.Join(tpl2Dir, "template.md"), []byte("# Built-in 2"), 0o600))

		tpl3Dir := filepath.Join(customDir, "custom-1")
		require.NoError(t, os.MkdirAll(tpl3Dir, 0o750))
		require.NoError(t, os.WriteFile(filepath.Join(tpl3Dir, "config.yaml"), []byte("name: custom-1\ncategory: test\ndescription: Custom template 1"), 0o600))
		require.NoError(t, os.WriteFile(filepath.Join(tpl3Dir, "template.md"), []byte("# Custom 1"), 0o600))
	...
	t.Run("only custom templates", func(t *testing.T) {
		...
		require.NoError(t, os.WriteFile(filepath.Join(tplDir, "config.yaml"), []byte("name: custom-only\ncategory: test\ndescription: Custom only"), 0o600))
		require.NoError(t, os.WriteFile(filepath.Join(tplDir, "template.md"), []byte("# Custom"), 0o600))
	...
	t.Run("empty custom directory", func(t *testing.T) {
		...
		require.NoError(t, os.MkdirAll(tplDir, 0o750))
		require.NoError(t, os.WriteFile(filepath.Join(tplDir, "config.yaml"), []byte("name: builtin\ncategory: test\ndescription: Built-in"), 0o600))
		require.NoError(t, os.WriteFile(filepath.Join(tplDir, "template.md"), []byte("# Built-in"), 0o600))
...
func TestLoadTemplate(t *testing.T) {
	t.Run("valid template", func(t *testing.T) {
		...
		require.NoError(t, os.WriteFile(filepath.Join(templateDir, "template.md"), []byte("# Test Template"), 0o600))
	...
	t.Run("template missing config.yaml", func(t *testing.T) {
		...
		require.NoError(t, os.MkdirAll(templateDir, 0o750))
		require.NoError(t, os.WriteFile(filepath.Join(templateDir, "template.md"), []byte("# Missing Config"), 0o600))
