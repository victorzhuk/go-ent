		...
		if err == nil && existing.Status == spec.LoopStatusRunning {
			return &mcp.CallToolResult{
				Content: []mcp.Content{&mcp.TextContent{
					Text: fmt.Sprintf("Loop already running. Use goent_loop_cancel to stop it first."),
				}},
			}, nil, nil
		}
	}
...
func loopSetHandler(ctx context.Context, req *mcp.CallToolRequest, input LoopSetInput) (*mcp.CallToolResult, any, error) {
	...
	if !store.LoopExists() {
		return &mcp.CallToolResult{
			Content: []mcp.Content{&mcp.TextContent{Text: "No loop state found. Use goent_loop_start first."}},
		}, nil, nil
	}
