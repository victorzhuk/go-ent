...
**Contents:**
- `pkg/platformspecs/` - Go code generated from YAML schemas
...
**Example:**
```go
// pkg/platformspecs/agent.go - Generated from YAML
type Agent struct {
    Name        string
    Description string
    Model       string
...
**Canonical Source:** `platformspecs/`
...
**Example (platformspecs/claude.yaml):**
```yaml
agent:
...
3. **Update Tests:**
   ```bash
   vim pkg/platformspecs/agent_test.go
   ```

4. **Run Tests:**
   ```bash
   go test ./pkg/platformspecs/...
   ```
...
- `pkg/platformspecs/*.go` - Generated from YAML schemas
...
| Modify schema | `platformspecs/` | `vim platformspecs/*.yaml` |
...
|-----------|-------|------|
| `pkg/` | ✅ Yes | Source code |
| `platformspecs/` | ✅ Yes | Canonical definitions |
| `.claude/` | ❌ No | Generated |
...
│                      Source Layer                            │
│  ┌──────────────┐  ┌──────────────┐  ┌──────────────┐      │
│  │  pkg/skills/ │  │   platformspecs/   │  │ internal/*/  │      │
│  │              │  │              │  │              │      │
...
**Remember:**
- ✅ Edit `pkg/` and `platformspecs/` freely
