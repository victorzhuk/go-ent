...
- **THEN** CLI binary is created at `bin/ent`
- **AND** binary is executable

#### Scenario: Direct Go build succeeds
- **WHEN** running `go build ./cmd/ent`
...
**What changed**: Consolidated templates from `/templates/` and `/cmd/ent/templates/` into `/internal/templates/` and fixed embedding of dotfiles (`.gitignore.tmpl`, `.golangci.yml.tmpl`) using explicit embed directives.
...
- **AND** no `/cmd/ent/templates/` copy directory
...
- **AND** no `/cmd/ent/templates/` artifacts remain
...
**What changed**: Removed empty `/cmd/ent/internal/resources/` directory that had no purpose or usage.

#### Scenario: No unused directories
- **WHEN** examining `/cmd/ent/internal/` structure
...
- **WHEN** listing directories in `/cmd/ent/internal/`
...
- **THEN** CLI is compiled and output to `bin/ent`
...
- **WHEN** checking `cmd/ent/` directory
